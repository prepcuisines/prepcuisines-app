...
export async function GET(req: NextRequest) {
  ...
  const { data: orders, error } = await supabase
    ...
    .select(
      'id, customer_id, status, items, total_amount, delivery_day, created_at, delivery_instructions, fulfilled, cancelled, ship_full_name, ship_phone, ship_house_number, ship_street, ship_postcode, ship_email, dpd_shipment_id, dpd_consignment_number, label_printed_at, menu_windows(week_start_date)'
    )
