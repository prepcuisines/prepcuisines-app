...
export default function AdminDashboard() {
  ...
  const refreshCurrentTab = async () => {
    ...
    try {
      ...
      if (tab === 'customers') jobs.push(loadCustomers(), loadMarketingLeads())
        ...
        jobs.push(loadOrders(), loadMapPoints(stokeRouteSingleDate || undefined))
  ...
  // The Delivery page has ONE date control (the chips/📅 at the top): the
  // map follows it too — a single picked date shows that day's pins, a
  // range or no selection shows everything.
  useEffect(() => {
    if (tab !== 'delivery' || !authenticated) return
    loadMapPoints(stokeRouteSingleDate || undefined)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tab, authenticated, printLabelsFrom, printLabelsTo])
